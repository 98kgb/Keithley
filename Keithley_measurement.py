...
for mode in range(1,6):
    ...
    for char_type in char_list:    
        ...
        for idx, v in enumerate(v_range):
            ...
            # SMU1A setting
            apply_V(inst1, ch_name='a', v=v, v_assigned=v1_a, char_type=char_type)
          
            # SMU1B setting
            apply_V(inst1, ch_name='b', v=v, v_assigned=v1_b, char_type=char_type)
                
            # SMU2A setting
            apply_V(inst2, ch_name='a', v=v, v_assigned=v2_a, char_type=char_type)
          
            # SMU2B setting
            apply_V(inst2, ch_name='b', v=v, v_assigned=v2_b, char_type=char_type)
        ...
        if mode <=2:
            ...
            if char_type == 'output':
                ...
                plt.plot(measures[:, 0], measures[:, 1], label=f"Vds = {v_fixed}V", marker='o', color='blue')
        ...
        if saving:
            ...
            if mode <= 2:
                ...
                df = pd.DataFrame(measures, columns=['Vds', 'Ids'])
                df[f'{column}'] = v_fixed
